...
The system SHALL expose GET /api/profiles/{handle}. The system MUST return the public profile for that handle when it exists and is not hidden by privacy rules. For private profiles, the system SHALL apply policy (e.g. 404 or restricted fields) as defined by the API contract.

#### Scenario: Get profile by handle returns public profile

- **WHEN** a client sends GET /api/profiles/{handle} for an existing public profile
- **THEN** system responds 200 with the Profile object
